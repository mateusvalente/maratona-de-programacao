(function () {
    ...
    <main class="catalog">
      ...
      <section class="band">
        <div class="filter-bar" style="grid-template-columns:repeat(2,minmax(180px,1fr))">
          <div class="field"><label for="draft-paradigm">Paradigma</label><select id="draft-paradigm"><option value="">Todos</option>${Object.entries(window.PARADIGM_LABELS).filter(([key]) => key !== "pre-processamento").map(([key, label]) => `<option value="${key}">${label}</option>`).join("")}</select></div>
  ...
  function render() {
    ...
    document.querySelector("#draft-grid").innerHTML = items.map((exercise) => `
      <article class="exercise-card" style="grid-template-columns:110px minmax(0,1fr) 190px">
        <span class="exercise-id">${exercise.platform}<br>${exercise.id}</span>
        <div class="exercise-main"><h2>${exercise.title}</h2><p><code>${exercise.key}.py</code></p><span class="status-badge waiting">tentativa · não validada</span></div>
        <div class="exercise-side"><a class="btn primary" href="${exercise.key}.py">Abrir .py</a><a class="btn" href="${exercise.url}" target="_blank" rel="noreferrer">Enunciado ↗</a></div>
      </article>`).join("");
  }
